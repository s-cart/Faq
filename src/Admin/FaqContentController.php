...
class FaqContentController extends RootAdminController
{
    public $languages;
    public $plugin;
    ...
    public function index()
    {
        ...
        $data = [
            ...
            'topMenuRight'  => [],
            'topMenuLeft'   => [],
            'urlDeleteItem' => sc_route_admin('admin_faq_content.delete'),
        ...
        $listTh = [
            ...
            'question'       => trans($this->plugin->pathPlugin.'::Content.question'),
        ...
        $arrSort = [
            ...
            'question__desc' => trans($this->plugin->pathPlugin.'::Content.admin.sort_order.question_desc'),
            'question__asc'  => trans($this->plugin->pathPlugin.'::Content.admin.sort_order.question_asc'),
        ];
        ...
        $dataTmp = (new AdminFaqContent)->getContentListAdmin($dataSearch);
        ...
        foreach ($dataTmp as $key => $row) {
            $dataTr[] = [
                ...
                'sort' => $row['sort'],
                'action' => '
                    <a href="' . sc_route_admin('admin_faq_content.edit', ['id' => $row['id']]) . '"><span title="' . trans($this->plugin->pathPlugin.'::Content.admin.edit') . '" type="button" class="btn btn-flat btn-primary"><i class="fa fa-edit"></i></span></a>&nbsp;
        ...
        //menuRight
        $data['menuRight'][] = '<a href="' . sc_route_admin('admin_faq_content.create') . '" class="btn  btn-success  btn-flat" title="New" id="button_create_new">
        ...
        $data['urlSort'] = sc_route_admin('admin_faq_content.index', request()->except(['_token', '_pjax', 'sort_order']));
        ...
        //menuSearch
        $data['topMenuRight'][] = '
            <form action="' . sc_route_admin('admin_faq_content.index') . '" id="button_search">
    ...
    public function create()
    {
        $data = [
            ...
            'url_action' => sc_route_admin('admin_faq_content.create'),
            'pathPlugin' => $this->plugin->pathPlugin,
        ];
        return view($this->plugin->pathPlugin.'::Admin.faq_content')
            ->with($data);
    }
    ...
    public function postCreate()
    {
        ...
        $data['alias'] = !empty($data['alias'])?$data['alias']:$data['descriptions'][$langFirst]['question'];
        $data['alias'] = sc_word_format_url($data['alias']);
        $data['alias'] = sc_word_limit($data['alias'], 100);
        $validator = Validator::make($data, [
            'sort' => 'numeric|min:0',
            'category_id' => 'required',
            'descriptions.*.question' => 'required|string|max:200',
            'descriptions.*.keyword' => 'nullable|string|max:200',
            'descriptions.*.description' => 'nullable|string|max:300',
            'alias' => 'required|regex:/(^([0-9A-Za-z\-_]+)$)/|string|max:100',
        ], [
            'descriptions.*.question.required' => sc_language_render('validation.required', 
            ['attribute' => sc_language_render($this->plugin->pathPlugin.'::Content.question')]),
        ...
        $content = AdminFaqContent::createContentAdmin($dataInsert);
        $id = $content->id;
        $dataDes = [];
        $languages = $this->languages;
        foreach ($languages as $code => $value) {
            $dataDes[] = [
                'content_id' => $id,
                'lang' => $code,
                'question' => $data['descriptions'][$code]['question'],
                'keyword' => $data['descriptions'][$code]['keyword'],
                'description' => $data['descriptions'][$code]['description'],
                'answer' => $data['descriptions'][$code]['answer'],
            ];
        }
        AdminFaqContent::insertDescriptionAdmin($dataDes);
        sc_clear_cache('cache_faq_content');
        return redirect()->route('admin_faq_content.index')
 ...
 */
    ...
    {
        $content = AdminFaqContent::getContentAdmin($id);
        ...
        $data = [
            ...
            'content' => $content,
            'categories' => (new AdminFaqCategory)->getTreeCategoriesAdmin(),
            'url_action' => sc_route_admin('admin_faq_content.edit', ['id' => $content['id']]),
            'pathPlugin' => $this->plugin->pathPlugin,
 ...
 */
    public function postEdit($id)
    {
        ...
        $validator = Validator::make($data, [
            ...
            'descriptions.*.question.required' => sc_language_render('validation.required', ['attribute' => trans($this->plugin->pathPlugin.'::Content.question')]),
        ]);
...
//Edit
        ...
        AdminFaqContent::insertDescriptionAdmin($dataDes);
        sc_clear_cache('cache_faq_content');
        return redirect()->route('admin_faq_content.index')->with('success', trans($this->plugin->pathPlugin.'::Content.admin.edit_success'));
 ...
 */
    ...
    public function checkPermisisonItem($id) {
        return AdminFaqContent::getContentAdmin($id);
    }
